<h1 align="center">Hi, I'm Adrián 👋</h1>

<h3 align="center">
  🧱 DevOps Engineer | Linux • Automation • Cloud-Native
</h3>

<p align="center">
  📍 Barcelona, Spain 🇪🇸 | 🌍 Open to Remote Opportunities
</p>

<p align="center">
  <b>10 years in Systems Administration evolving into Modern DevOps.</b><br>
  I design, automate and operate production-like environments.<br>
  Focused on <b>resilient infrastructure, CI/CD, observability and infrastructure as code</b>.
</p>

<div align="center">
  <img height="350" src="https://res.cloudinary.com/dtmrroz8t/image/upload/v1747772320/user__2_funba7.gif" alt="Adrian working" />
</div>

---

<h2 align="center">🚀 What I'm Building Right Now</h2>

<p align="center">
  I'm developing a <b>Production-Grade VPS Lab</b> to simulate real-world DevOps environments:
</p>

<p align="center">
  🐧 Hardened Linux Server (SSH, firewall, fail2ban, automation)<br>
  🐳 Dockerized API + Reverse Proxy<br>
  ☸️ Kubernetes local cluster for orchestration practice<br>
  ☁️ Terraform-managed infrastructure<br>
  📊 Observability stack (Prometheus + Grafana + centralized logs)<br>
  🔁 CI/CD automation via GitHub Actions<br>
  🛡️ DevSecOps fundamentals applied
</p>

<p align="center">
  Everything documented with:<br>
  📘 Runbooks • 📜 ADRs • 🧯 Postmortems • 🛠️ Troubleshooting guides
</p>

---

<h2 align="center">🛠️ Core Stack</h2>

<div align="center">

  <b>🧠 Systems & Automation</b><br><br>
  <img src="https://img.shields.io/badge/Linux-FCC624?style=for-the-badge&logo=linux&logoColor=black" />
  <img src="https://img.shields.io/badge/Bash-121011?style=for-the-badge&logo=gnu-bash&logoColor=white" />
  <img src="https://img.shields.io/badge/Systemd-000000?style=for-the-badge" />
  <img src="https://img.shields.io/badge/Networking-00599C?style=for-the-badge" />
  <img src="https://img.shields.io/badge/SSH-000000?style=for-the-badge" />
  <br><br>

  <b>🐳 Containers & Orchestration</b><br><br>
  <img src="https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=docker&logoColor=white" />
  <img src="https://img.shields.io/badge/Docker_Compose-384D54?style=for-the-badge&logo=docker&logoColor=white" />
  <img src="https://img.shields.io/badge/Kubernetes-326CE5?style=for-the-badge&logo=kubernetes&logoColor=white" />
  <img src="https://img.shields.io/badge/Helm-0F1689?style=for-the-badge&logo=helm&logoColor=white" />
  <br><br>

  <b>☁️ Infrastructure as Code</b><br><br>
  <img src="https://img.shields.io/badge/Terraform-7B42BC?style=for-the-badge&logo=terraform&logoColor=white" />
  <img src="https://img.shields.io/badge/Modular_Infrastructure-5C4EE5?style=for-the-badge" />
  <img src="https://img.shields.io/badge/Environment_Separation-6C63FF?style=for-the-badge" />
  <br><br>

  <b>📊 Observability</b><br><br>
  <img src="https://img.shields.io/badge/Prometheus-E6522C?style=for-the-badge&logo=prometheus&logoColor=white" />
  <img src="https://img.shields.io/badge/Grafana-F46800?style=for-the-badge&logo=grafana&logoColor=white" />
  <img src="https://img.shields.io/badge/Loki-2C3E50?style=for-the-badge" />
  <img src="https://img.shields.io/badge/Alerting-FF4D4D?style=for-the-badge" />
  <br><br>

  <b>💻 Development</b><br><br>
  <img src="https://img.shields.io/badge/Node.js-43853D?style=for-the-badge&logo=node.js&logoColor=white" />
  <img src="https://img.shields.io/badge/TypeScript-007ACC?style=for-the-badge&logo=typescript&logoColor=white" />
  <img src="https://img.shields.io/badge/REST_API-005571?style=for-the-badge" />
  <img src="https://img.shields.io/badge/Clean_Architecture-222222?style=for-the-badge" />

<div>


<div align="center">
  <a href="https://www.linkedin.com/in/adcanoar/">
    <img src="https://img.shields.io/badge/Connect_on-LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" />
  </a>
  <br><br>
  <i>"Infrastructure is invisible when it's done right." 🚀</i>
</div>
